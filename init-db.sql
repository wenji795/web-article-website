...
ALTER TABLE web_comment ADD COLUMN parent_comment_id INT NULL;
ALTER TABLE web_comment ADD FOREIGN KEY (parent_comment_id) REFERENCES web_comment(comment_id) ON DELETE CASCADE;
SELECT c.comment_id, c.article_id, c.comment_content, c.create_date, c.parent_comment_id, u.user_name
FROM web_comment c
         JOIN web_user u ON c.user_id = u.user_id
WHERE c.article_id = ? -- 这里的问号应该被替换为实际的文章ID
ORDER BY c.create_date DESC;

create table web_like (
    web_like_id INT AUTO_INCREMENT,
    user_id int(11) not null,
    user_name char(20) not null,
    article_id int(11) not null,
#     article_title varchar(256) not null,
    create_date datetime NOT NULL,
    primary key (web_like_id),
    FOREIGN KEY (article_id) REFERENCES web_article(article_id),
    FOREIGN KEY (user_id) REFERENCES web_user(user_id)
);
ALTER TABLE web_like MODIFY COLUMN user_name VARCHAR(255) DEFAULT NULL;
...
# DESCRIBE web_like;
SHOW FIELDS FROM web_like WHERE Field = 'user_name';
ALTER TABLE web_like MODIFY COLUMN user_name VARCHAR(255) DEFAULT NULL;
